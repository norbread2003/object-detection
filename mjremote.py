...
class mjremote:
    ...
    def getinput(self):
        ...
        self._s.sendall(struct.pack(integer_packing, 1))
        data = bytearray(40)
        self._recvall(data)
    ...
    def getimage(self, buffer):
        if not self._s:
            return 'Not connected'
        self._s.sendall(struct.pack(integer_packing, 2))
        self._recvall(buffer)

    def savesnapshot(self):
        if not self._s:
            return 'Not connected'
        self._s.send(struct.pack(integer_packing, 3))

    def savevideoframe(self):
        if not self._s:
            return 'Not connected'
        self._s.send(struct.pack(integer_packing, 4))

    def setcamera(self, index, h, w):
        if not self._s:
            return 'Not connected'
        self._s.sendall(struct.pack(integer_packing, 5))
        self._s.sendall(struct.pack(integer_packing, index))
        self._s.sendall(struct.pack(integer_packing, h))
        self._s.sendall(struct.pack(integer_packing, w))
        data = bytearray(4)
        self._recvall(data)
        print(data)
    ...
    def setqpos(self, qpos):
        ...
        self._s.sendall(struct.pack(integer_packing, 6))
    ...
    def setmocap(self, pos, quat):
        ...
        self._s.sendall(struct.pack(integer_packing, 7))
